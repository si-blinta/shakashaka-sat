"""CNF encoding of the constraints defined in the paper.

The implementation follows the same five families: initialization and
uniqueness (1), clue constraints (2), L-shape closure (3), triangle
adjacency (4), and binary exclusions (5).
"""
...
class SATEncoder:
    ...
    def encode(self) -> list[list[int]]:
        ...
        self._encode_at_most_one()          # (1c)
        self._encode_at_least_one()         # (1d)
        self._encode_border_walls()         # (1a)
        self._encode_no_black_inside()      # (1b)
        self._encode_diagonal_continuity()  # (5a-5b)
        self._encode_propagation_1()        # (3a-3d)
        self._encode_propagation_2()        # (4a-4h)
        self._encode_propagation_3()        # (3e-3h)
        self._encode_binary_exclusion()     # (5c-5h)
        self._encode_index_constraints()    # (2a-2b)
        return self.clauses
    ...
    # ---- (1d) At-least-one motif per cell ----
    ...
    # ---- (1b) No black for playable cells ----
    ...
    # ---- (3e-3h) L-shape closure with three white cells ----
    def _encode_propagation_3(self):
        nr, nc = self.nrows, self.ncols
        # (3e): x_{i,j,5} ∧ x_{i,j+1,5} ∧ x_{i+1,j,5} => x_{i+1,j+1,5} ∨ x_{i+1,j+1,3}
        for i in range(1, nr):
            for j in range(1, nc):
                self._add_clause([
                    -self.var(i, j, 5),
        ...
        # (3f): x_{i,j,5} ∧ x_{i,j+1,5} ∧ x_{i+1,j+1,5} => x_{i+1,j,5} ∨ x_{i+1,j,2}
        for i in range(1, nr):
            for j in range(1, nc):
                self._add_clause([
                    -self.var(i, j, 5),
        ...
        # (3g): x_{i,j,5} ∧ x_{i+1,j,5} ∧ x_{i+1,j-1,5} => x_{i,j-1,5} ∨ x_{i,j-1,1}
        for i in range(1, nr):
            for j in range(2, nc + 1):
                self._add_clause([
                    -self.var(i, j, 5),
        ...
        # (3h): x_{i,j,5} ∧ x_{i+1,j,5} ∧ x_{i+1,j+1,5} => x_{i,j+1,5} ∨ x_{i,j+1,4}
        for i in range(1, nr):
            for j in range(1, nc):
                self._add_clause([
                    -self.var(i, j, 5),
    ...
    # ---- (5c-5h) Binary exclusions and rotations ----
    ...
    def _encode_index_constraints(self):
        for (i, j), m in self.puzzle.indexed_cells.items():
            ...
            # (2b) At least m triangles: for every subset of size d-m+1,
